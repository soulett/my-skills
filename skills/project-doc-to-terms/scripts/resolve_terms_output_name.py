...
def build_output(source_path: Path, title: str) -> dict[str, str]:
    directory = source_path.parent if source_path.suffix else source_path
    return {
        "project_title": title,
        "terms_output": str(directory / f"{title}_专业名词详解文档.md"),
    }


def main() -> None:
    parser = argparse.ArgumentParser(description="Resolve the output filename for a terminology explanation document.")
    parser.add_argument("source", help="Source file path or directory path.")
    args = parser.parse_args()

    source_path = Path(args.source).expanduser().resolve()
    title = derive_title(source_path)
    print(json.dumps(build_output(source_path, title), ensure_ascii=False, indent=2))
